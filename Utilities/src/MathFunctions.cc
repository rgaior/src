...
double Integrate(vector<double>& v, double a, double b) {
  ...
  //rom comment:
  // extended or composite Simpson rule for integration.
  // the steps are chosen in log but the function is integrated in linear ( exp(log) )
  // - variable change t = ln(x)
  // - integration method: int_x1^xn f(x)dx = step*[ (1/3)f_1 + (4/3)f_2 + (2/3)f_3 + .... + (4/3)f_N-1 + (1/3)f_N] + O(1/N^4)
